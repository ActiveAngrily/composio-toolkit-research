...
VERDICT_LABEL = {
    ...
    "absence-claim": "asserts an absence — cannot be quoted",
    "abstained": "no answer claimed",
    "no-quote": "claimed without a quote — quarantined",
    "not-asked-pass1": "pass 1 never asked for a quote here",
...
def page(rows, pat, cov, delta, xc, ua) -> str:
    ...
    quarantined = [(r["app"], f, d["f"][f]["v"]) for r in rows for f, d in [(None, r)] if False]
    # Real quarantine exhibits come from the dataset's own record of them.
    exhibits = []
    for r in rows:
...
.ev{{cursor:help;font-size:10px;padding:1px 5px;border-radius:4px;margin-left:5px}}
.ev-y{{background:#13291f;color:#79c39c}} .ev-n{{background:#2a1e1a;color:#dc9a8f}}
.ev-q{{background:#241a1a;color:#e0847a}} .ev-t{{background:#1f2530;color:var(--dim)}}
...
<section><div class=wrap>
  ...
  <h2>Four findings, with their numbers</h2>
...
<section><div class=wrap>
  ...
  <table id=matrix><thead><tr>
    <th>App</th><th>Auth</th><th>Access</th><th>Verdict</th><th>Blocker</th>
    <th>Breadth</th><th>MCP</th><th>Composio</th>
  </tr></thead><tbody></tbody></table>
  <p class=meta>Click any row for every claim, its quote, its source, its tier and its
  validation verdict.</p>
</div></section>
...
<section><div class=wrap>
  ...
  <div class=two style="margin-top:18px">
    <div>
      ...
      verified on a page owned by the vendor (tier&nbsp;1&ndash;2).</p>
    </div>
    <div>
...
<section><div class=wrap>
  ...
  <div class=two>
    ...
    <div class=card><h3>Apps that defeated us</h3>
      ...
      The correct finding is &ldquo;wrap the CLI&rdquo;, and the first schema could not express
      it, so the model invented an API for one of them.</p>
...
function evBadge(f){{
  ...
  const cls=f.e?'ev-y':(f.g==='QUOTE_NOT_FOUND'||f.g==='no-quote')?'ev-q':
            (f.g==='truncated-evidence')?'ev-t':'ev-n';
  const txt=f.e?'t'+f.t:(f.g==='QUOTE_NOT_FOUND'?'!':f.g==='truncated-evidence'?'?':'~');
  return `<span class="ev ${{cls}}" title="${{esc(LAB[f.g]||f.g)}} (source tier ${{f.t}})">${{txt}}</span>`;
}}
...
function render(list){{
  ...
  tb.innerHTML=list.map(r=>`<tr data-id=${{r.id}}>
    <td><strong>${{esc(r.app)}}</strong><div class=mono style="color:var(--faint)">${{esc(r.cat)}}</div></td>
    ...
    <td>${{dash(r.blk==='none'?'':r.blk)}}</td>
    <td>${{dash(r.brd)}}</td>
    <td>${{dash(r.f.existing_mcp.v)}}${{evBadge(r.f.existing_mcp)}}</td>
  ...
  document.getElementById('count').textContent=list.length+' of 100 shown';
